...
function startStatusBridge({ runtimeDir = process.env.XDG_RUNTIME_DIR, snapshot = () => ({}), controlTimeoutMs = 3000 } = {}) {
  ...
  const waitFor = (target, baseline) => new Promise(resolve => { const watcher = () => { if (sequence > baseline && current.hands_free === target) { clearTimeout(timer); watchers.delete(watcher); resolve(true); } }; const timer = setTimeout(() => { watchers.delete(watcher); resolve(false); }, controlTimeoutMs); watchers.add(watcher); });
