...
function sanitize(input) { const state = STATES.has(input?.state) ? input.state : "error", meter = input?.microphone; const microphone = { sequence: Number.isSafeInteger(meter?.sequence) && meter.sequence >= 0 ? meter.sequence : 0, capture: "unavailable", rms: null }; if (meter?.capture === "available" && Number.isFinite(meter?.rms) && meter.rms >= 0 && meter.rms <= 1) { microphone.capture = "available"; microphone.rms = meter.rms; } const result = { state, hands_free: input?.hands_free === true, microphone }; if (state === "error" && /^[a-z0-9_.-]{1,64}$/.test(input?.error || "")) result.error = input.error; return result; }
...
function startStatusBridge({ runtimeDir = defaultRuntimeDir(), snapshot = () => ({}), controlTimeoutMs = 3000, heartbeatMs = 1000 } = {}) {
  ...
  const publish = next => { current = sanitize({ ...next, microphone: next?.microphone ?? current.microphone }); sequence += 1; const value = packet(); for (const socket of clients) write(socket, value); for (const watcher of watchers) watcher(); };
  const publishMeter = meter => { let microphone; if (meter?.capture === "unavailable" && meter.rms === undefined) microphone = { sequence: current.microphone.sequence + 1, capture: "unavailable", rms: null }; else if (meter?.capture === "available" && Number.isFinite(meter.rms) && meter.rms >= 0 && meter.rms <= 1) microphone = { sequence: current.microphone.sequence + 1, capture: "available", rms: meter.rms }; else return false; publish({ ...current, microphone }); return true; };
