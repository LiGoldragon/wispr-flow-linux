...
function startStatusBridge({ runtimeDir = process.env.XDG_RUNTIME_DIR, snapshot = () => ({}), controlTimeoutMs = 3000 } = {}) {
  ...
  const clients = new Set(), watchers = new Set(), owned = new Map();
  ...
  const waitFor = target => new Promise(resolve => { if (current.hands_free === target) return resolve(true); const watcher = () => { if (current.hands_free === target) { clearTimeout(timer); watchers.delete(watcher); resolve(true); } }; const timer = setTimeout(() => { watchers.delete(watcher); resolve(false); }, controlTimeoutMs); watchers.add(watcher); });
  ...
  const controlServer = net.createServer(socket => { let buffer = ""; socket.on("error", () => socket.destroy()); socket.on("data", chunk => { buffer += chunk; const end = buffer.indexOf("\n"); if (end < 0) return buffer.length > 4096 && socket.destroy(); let request; try { request = JSON.parse(buffer.slice(0, end)); } catch { socket.end(`${JSON.stringify({ contract: CONTRACT, type: "control_result", ok: false, error: "invalid_request" })}\n`); return; }
    queue = queue.then(async () => { const reply = { contract: CONTRACT, type: "control_result", id: typeof request.id === "string" ? request.id : "", ok: false }; if (request.contract !== CONTRACT || request.type !== "control" || request.command !== "toggle_hands_free") reply.error = "invalid_request"; else if (!action) reply.error = "unavailable"; else try { const result = await action(); if (typeof result?.hands_free !== "boolean") reply.error = result?.error === "not_toggleable" ? "not_toggleable" : "action_failed"; else if (await waitFor(result.hands_free)) { reply.ok = true; reply.hands_free = result.hands_free; } else reply.error = "state_timeout"; } catch { reply.error = "action_failed"; } socket.end(`${JSON.stringify(reply)}\n`); }); }); });
