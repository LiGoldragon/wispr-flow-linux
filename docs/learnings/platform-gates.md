...
| Synced macOS profiles carried an unmappable `-1` PTT key → normalize only a shortcut map with no matchable PTT binding to Ctrl+Meta (`162+91`) at its startup update | [`linux-ptt-shortcut.sh`](../../scripts/patches/linux-ptt-shortcut.sh) | `WISPR_LINUX_PTT_SHORTCUT` |
