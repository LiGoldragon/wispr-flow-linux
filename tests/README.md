...
The same Linux patch and shipped-asar marker suites are exposed as
`nix flake check`'s `wispr-flow-linux-patches` check.

| File | Covers |
|------|--------|
| `launcher-common.bats` | `scripts/launcher-common.sh`: logging paths, `check_display`, `detect_display_backend`, `build_electron_args` (sandbox/GPU/backend flag selection), `setup_electron_env`, `cleanup_stale_lock`, `wispr_config_dir`. |
