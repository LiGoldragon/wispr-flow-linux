...
(async () => {
  ...
  assert.deepEqual(Object.keys(updated).sort(),
    ["contract", "hands_free", "microphone", "sequence", "session_id", "state", "type"]);
  await bridge.close();
...
const bridge = startStatusBridge({ runtimeDir: runtime, heartbeatMs: 10, snapshot: () => ({ state: "recording", hands_free: false }) });
...
(async () => { await bridge.ready; const initial = await next(); assert.equal(initial.microphone.capture, "unavailable"); assert.equal(initial.microphone.rms, null); assert.equal(bridge.publishMeter({ capture: "available", rms: 0 }), true); const silence = await next(); assert.equal(silence.microphone.capture, "available"); assert.equal(silence.microphone.rms, 0); const microphoneSequence = silence.microphone.sequence; const sequence = silence.sequence; assert.equal(bridge.publishMeter({ capture: "available", rms: Infinity }), false); const retained = await next(); assert.equal(retained.microphone.rms, 0); assert.equal(retained.microphone.sequence, microphoneSequence); assert.equal(retained.sequence, sequence); assert.equal(Object.keys(retained).includes("packet"), false); assert.equal(Object.keys(retained).includes("samples"), false); assert.equal(bridge.publishMeter({ capture: "available", rms: 0 }), true); const freshSilence = await next(); assert.equal(freshSilence.microphone.rms, 0); assert.ok(freshSilence.microphone.sequence > microphoneSequence); assert.equal(bridge.publishMeter({ capture: "available", rms: .5 }), true); const heartbeat = await new Promise((resolve, reject) => { const socket = net.connect(bridge.statusPath); let lines = ""; const timeout = setTimeout(() => reject(new Error("meter heartbeat did not arrive")), 200); socket.on("data", chunk => { lines += chunk; const packets = lines.trim().split("\n"); if (packets.length >= 2) { clearTimeout(timeout); socket.destroy(); resolve(JSON.parse(packets.at(-1))); } }); socket.on("error", reject); }); assert.equal(heartbeat.microphone.capture, "available"); assert.equal(heartbeat.microphone.rms, .5); assert.equal(heartbeat.microphone.sequence, freshSilence.microphone.sequence + 1); assert.ok(heartbeat.sequence > sequence); const heartbeatSequence = heartbeat.microphone.sequence; const nextHeartbeat = await new Promise((resolve, reject) => { const socket = net.connect(bridge.statusPath); let lines = ""; socket.on("data", chunk => { lines += chunk; const packets = lines.trim().split("\n"); if (packets.length >= 2) { socket.destroy(); resolve(JSON.parse(packets.at(-1))); } }); socket.on("error", reject); }); assert.equal(nextHeartbeat.microphone.sequence, heartbeatSequence); assert.ok(nextHeartbeat.sequence > heartbeat.sequence); assert.equal(bridge.publishMeter({ capture: "unavailable" }), true); const reset = await next(); assert.equal(reset.microphone.capture, "unavailable"); assert.equal(reset.microphone.rms, null); const resetSequence = reset.microphone.sequence; const resetHeartbeat = await new Promise((resolve, reject) => { const socket = net.connect(bridge.statusPath); let lines = ""; socket.on("data", chunk => { lines += chunk; const packets = lines.trim().split("\n"); if (packets.length >= 2) { socket.destroy(); resolve(JSON.parse(packets.at(-1))); } }); socket.on("error", reject); }); assert.equal(resetHeartbeat.microphone.sequence, resetSequence); await bridge.close(); })().catch(error => { console.error(error); process.exitCode = 1; });
NODE
	[[ $status -eq 0 ]]
}
